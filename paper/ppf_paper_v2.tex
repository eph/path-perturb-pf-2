...
\paragraph{Recommended auxiliary proposal for mixture indices.}
In the sampling (non-branching) RBPF variant, a high-leverage default is
\[
q_t(j)\propto w_t^{(j)}\,p(y_t\mid y_{1:t-1},j),
\]
where $j=(s,k)$ and the predictive likelihood $p(y_t\mid y_{1:t-1},j)$ is available from the Kalman predict step under component $j$.
This targets mixture components that both have nontrivial prior weight and are consistent with the current observation, often improving ESS at negligible additional cost.
