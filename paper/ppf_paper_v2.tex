...
In the NK--ELB experiment, the OccBin extended-path horizon is $H=20$ and we simulate a dataset of length $T=250$ with small independent measurement noise on $(\pi_t,x_t,i_t)$.
...
The PPPF-mixture-RBPF delivers higher ESS and lower Monte Carlo variability of the likelihood estimator at a lower runtime cost, consistent with the variance-reduction role of Rao--Blackwellization.
However, in this minimal implementation the mean log-likelihood differs substantially from the OccBin-based baseline, reflecting approximation bias from finite-horizon, local-linearization, and low-dimensional quadrature choices.
...
\midrule
OccBin + bootstrap PF & 492.3 & 949.2 & 115.6 & 2.5 & 17.3 \\
PPPF-mixture + RBPF & $-19640.3$ & 171.4 & 67.7 & 0.6 & 100.6 \\
\bottomrule
\end{tabular}
\caption{NK--ELB benchmark (reference implementation): likelihood variability across $R=30$ repeated particle-filter runs on fixed data, with $T=250$ and $N=256$ particles.}
